...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.myPlanner.gmaps = googlemaps.Client(key=
            'AIzaSyDBO8sGBqk8aAs9OhvTdpsr0rSEURemOVk')
        self.myplaces = self.myPlanner.findPlaces([3, 3, 3, 5, 3, 3, 3, 3, 5, 3, 3, 3]
            , "Reno", 1600)
    ...
    def test_findLocationByType(self):
        self.assertEqual(self.myPlanner.findLocationByType("Las Vegas", 
            "restaurant", 1000)['results'][0]['vicinity'], 
            '129 East Fremont Street, Las Vegas')
        
    def test_findPlaces(self):
        self.assertEqual(self.myPlanner.findPlaces([5, 5, 3, 3, 3, 3, 3, 5, 3, 3, 3, 3]
                , "Reno", 40000)[0]['results'][1]['vicinity'], 
                '407 North Virginia Street, Reno')

    def test_showItinerary(self):
        self.assertEqual(self.myPlanner.showItinerary(self.myplaces)[0], 
            ['Legacy Vacation Resort Reno', '140 Court Street, Reno'])

    def test_showSanFranciscoItinerary(self):
        places = self.myPlanner.findPlaces([5, 5, 3, 3, 3, 3, 3, 5, 3, 3, 3, 3],
                 "San Francisco")
        self.assertEqual(self.myPlanner.showItinerary(places)[1:3], 
    ...
    def test_RenoStadiumItinerary(self):
        self.assertEqual(self.myPlanner.findPlaces('Reno', 
            self.myPlanner.recommend([1, 2, 1, 3, 2, 3, 3, 5, 2, 3, 3, 3])
            [0])[0]['results'][1]['vicinity'], 
    ...
    def test_ParkItineraryInArizona(self):
        ...
        self.assertEqual(self.myPlanner.showItinerary(parks, 3), 
